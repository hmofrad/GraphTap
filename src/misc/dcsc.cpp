/*
 * dcsc.cpp: DCSC SpMV implementation (LA3)
...
void classification_dcsc(uint32_t num_vertices)
{
...
void init_dcsc_regulars(uint32_t nnz_, uint32_t ncols_)
{
...
void spmv_dcsc_regulars(uint32_t offset)
{
    uint32_t ncols = 0;
    if(offset)
...
void init_dcsc_sources(uint32_t nnz_, uint32_t ncols_)
{
...
void run_dcsc()
{
    ...
    init_dcsc_regulars(triples_regulars->size(), nnz_ingoings);
    popu_dcsc_regulars();
    init_dcsc_sources(triples_sources->size(), nnz_outgoings);
    popu_dcsc_sources();    
    printf("[x]Compression is done\n");
}
void done_dcsc();
void spmv_dcsc_sources(uint32_t offset)
{
    uint32_t ncols = 0;
    if(offset)
...
void spmv_dcsc()
{
    spmv_dcsc_regulars(0);
    spmv_dcsc_regulars(regulars_sinks_offset);

    spmv_dcsc_sources(0);
    spmv_dcsc_sources(sources_sinks_offset);
