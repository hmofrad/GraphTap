/*
 * dcsc_d.cpp: DCSC SpMSpV implementation
...
void init_dcsc(uint32_t nnz_, uint32_t ncols)
{
    ...
    size =  nnz * (sizeof(uint32_t) + sizeof(uint32_t)) + (ncols_plus_one * sizeof(uint32_t)) + (nnz_cols_ * sizeof(uint32_t)) + (nnz_rows_ * sizeof(uint32_t));
...
void done_dcsc()
{
    ...
    for(uint32_t i = 0; i < nnz_rows_; i++)
